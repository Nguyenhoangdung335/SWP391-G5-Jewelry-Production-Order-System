      ...
      <Modal show={showModal} size="lg" backdrop="static" keyboard={false}>
        ...
        <Modal.Body style={{ width: "100%" }}>
          <CreateRequest
            ...
            handleCancelRequest={handleRequestCanceled}
          />
        </Modal.Body>
      </Modal>
    </Container>
...
const RenderSpecificationForm = ({ handleSubmit }) => {
    ...
    if (formState.selectedMetalName) {
      ...
      setFormState((prev) => ({
        ...
        selectedMetal: metalData.find(
          (metal) =>
            metal.name === formState.selectedMetalName &&
            metal.unit === units[0]
        ),
  ...
  const handleChangeMetalUnit = (e) => {
    ...
    if (formState.selectedMetalName) {
      const chosenMetal = metalData.find((item) => {
        console.log(item);
        console.log(
  ...
  const handleChangeGemstone = useCallback(
    (e) => {
      const { name, value } = e.target;
      if (value === "") {
  ...
  return (
    <Form
      onSubmit={(event) => handleSubmit(event, formState, selectedGemstoneProp)}
      className="mb-5"
    >
      <Container>
        <Row>
        ...
        <>
          ...
          {formState.selectedMetalName && (
            <>
              ...
              <MetalWeight
                onChange={handleChange}
                selectedUnit={formState.selectedMetalUnit}
                value={formState.selectedMetalWeight}
              />
...
export { RenderSpecificationForm };
