...
import { ChainType, DesignStyle, GemstoneForm, ImageInput, JewelryType, Length, MetalForm, Occasion, Texture } from "../orderFlows/DesignOptionForms";
import axios from "axios";
import ServerUrl from "../reusable/ServerUrl";
import ResizeImage from "../reusable/ResizeImage";
import { useAlert } from "../provider/AlertProvider";

const ProductEditor = ({product = defaultProduct, isShow, onHide, fetchData}) => {
    ...
    useEffect(() => {
        setFormState({
            ...
            selectedOccasion: product?.specification? product.specification.occasion :"",
            selectedLength: product?.specification? product.specification.length :"",
            selectedTexture: product?.specification? product.specification.texture :"",
            selectedChainType: product?.specification? product.specification.chainType :"",
        ...
        setSelectedMetalProp({
            selectedMetalName: product?.specification?.metal? product.specification.metal.name: "",
            selectedMetalUnit: product?.specification?.metal? product.specification.metal.unit: "",
            selectedMetal: product?.specification? product.specification.metal: null,
            selectedMetalWeight: product?.specification? product.specification.metalWeight: "",
        });
        setSelectedGemstoneProp({
            selectedGemstoneName: product?.specification?.gemstone? product.specification.gemstone.name: "",
            selectedGemstoneShape: product?.specification?.gemstone? product.specification.gemstone.shape: "",
            selectedGemstoneCut: product?.specification?.gemstone? product.specification.gemstone.cut: "",
            selectedGemstoneClarity: product?.specification?.gemstone? product.specification.gemstone.clarity: "",
            selectedGemstoneColor: product?.specification?.gemstone? product.specification.gemstone.color: "",
            selectedGemstoneWeight: product?.specification? product.specification.gemstoneWeight: "",
            selectedGemstone: product?.specification? product.specification.gemstone: null,
        })
    }, [product]);
    ...
    const handleSubmitImage = async () => {
        if (formState.imageFile && formState.imageFile instanceof File) {
            ...
            const response = await axios.post(`${ServerUrl}/api/products/image`,
                formData,
                {
    ...
    const handleSubmit = async () => {
        ...
        console.log(formState.imageURL);
        setTimeout(async () => { // Add a slight delay to ensure the state is updated before proceeding
            const requestBody = {
                ...product,
                name: formState.name,
            ...
            if (product?.id && product?.id !== "")
                ...
                request = {...createRequest, data: requestBody};
    ...
    return (
        <Modal size="xl" fullscreen="xl-down" centered keyboard animation show={isShow} >
            <Modal.Header closeButton style={{ width: "100%", }} onHide={onHide} ><h4>Product Editor</h4></Modal.Header>
            <Modal.Body style={{ width: "100%", }}>
                <Container>
                    <Row>
                        <Col sm={5} style={{height: "70vh", overflowY: "auto"}}>
                            <ImageInput label="Product Image" initialImageUrl={formState.imageURL} onChange={handleChangeImage  } urlName="imageURL" fileName="imageFile" />
                            <Row className="mb-3">
                                <ProductNameInput value={formState.name} onChange={handleChange} name="name" />
                            </Row>
                            <Row className="mb-3">
                                <ProductDescriptionInput value={formState.description} onChange={handleChange} name="description" />
                            </Row>
                        </Col>
            ...
            <Modal.Footer>
                ...
                <Button type={"button"} variant="primary" onClick={handleSubmit} disable={handleDisable}>Confirm</Button>
            </Modal.Footer>
        </Modal>
    );
};
